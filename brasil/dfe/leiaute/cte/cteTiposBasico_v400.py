...
class TModTranspGTVe(str):
    ...
    _restriction = Restriction(base=r"xs:string", white_space=r"preserve", enumeration=['01', '06'])
    pass
...
class TFinGTVe(str):
    ...
    _restriction = Restriction(base=r"xs:string", white_space=r"preserve", enumeration=['4'])
    pass
...
class TGTVe(Element):
    ...
    class infCte(ComplexType):
        ...
        class ide(ComplexType):
            ...
            _choice = [['toma', 'tomaTerceiro']]
            cUF: TCodUfIBGE = Element(TCodUfIBGE, documentation=['Código da UF do emitente da GTV-e.', 'Utilizar a Tabela do IBGE.'])
            ...
            mod: TModGTVe = Element(TModGTVe, documentation=['Modelo do documento fiscal', 'Utilizar o código 64 para identificação do CT-e Guia de Transporte de Valores'])
            serie: str = Element(str, documentation=['Série da GTV-e', 'Preencher com "0" no caso de série única'])
            nCT: TNF = Element(TNF, documentation=['Número da GTV-e'])
            dhEmi: str = Element(str, documentation=['Data e hora de emissão da GTV-e', 'Formato AAAA-MM-DDTHH:MM:DD TZD'])
            tpImp: str = Element(str, documentation=['Formato de impressão do DACTE', 'Preencher com: 1 - Retrato; 2 - Paisagem.'])
            tpEmis: str = Element(str, documentation=['Forma de emissão da GTV-e', 'Preencher com:\n1 - Normal;\n 2- Contingencia offline \n7 - Autorização pela SVC-RS;\n 8 - Autorização pela SVC-SP'])
            cDV: str = Element(str, documentation=['Digito Verificador da chave de acesso da GTV-e', 'Informar o dígito  de controle da chave de acesso do CT-e, que deve ser calculado com a aplicação do algoritmo módulo 11 (base 2,9) da chave de acesso. '])
            tpAmb: TAmb = Element(TAmb, documentation=['Tipo do Ambiente', 'Preencher com:1 - Produção; 2 - Homologação'])
            tpCTe: str = Element(str, documentation=['Tipo da GTV-e', 'Preencher com:\n 4 - GTV-e'])
            verProc: str = Element(str, documentation=['Versão do processo de emissão', 'Iinformar a versão do aplicativo emissor de CT-e.'])
            cMunEnv: TCodMunIBGE = Element(TCodMunIBGE, documentation=['Código do Município de envio da GTV-e (de onde o documento foi transmitido)', 'Utilizar a tabela do IBGE. Informar 9999999 para as operações com o exterior.'])
            xMunEnv: str = Element(str, documentation=['Nome do Município de envio da GTV-e (de onde o documento foi transmitido)', 'Informar PAIS/Municipio para as operações com o exterior.'])
            UFEnv: TUf = Element(TUf, documentation=['Sigla da UF de envio da GTV-e (de onde o documento foi transmitido)', "Informar 'EX' para operações com o exterior."])
            modal: str = Element(str, documentation=['Modal da GTV-e', 'Preencher com:\n01-Rodoviário \n06-Multimodal'])
            ...
            dhSaidaOrig: str = Element(str, documentation=['Data e hora de saida da origem', 'Formato AAAA-MM-DDTHH:MM:DD TZD'])
            dhChegadaDest: str = Element(str, documentation=['Data e hora de chegada no destino', 'Formato AAAA-MM-DDTHH:MM:DD TZD'])
            ...
            toma: toma = Element(toma, documentation=['Indicador do "papel" do tomador do serviço no GT-e'])

            class tomaTerceiro(ComplexType):
            ...
            tomaTerceiro: tomaTerceiro = Element(tomaTerceiro, documentation=['Indicador do "papel" do tomador do serviço no CTV-e'])
        ...
        ide: ide = Element(ide, documentation=['Identificação da GTV-e '])
        ...
        class compl(ComplexType):
            ...
            xEmi: str = Element(str, documentation=['Funcionário emissor da GTV-e'])
...
Informar o nome do campo no atributo xCampo e o conteúdo do campo no XTexto"""
                ...
                def add(self, xTexto=None, xCampo=None) -> TGTVe.infCte.compl.ObsCont:
...
Informar o nome do campo no atributo xCampo e o conteúdo do campo no XTexto"""
                ...
                def add(self, xTexto=None, xCampo=None) -> TGTVe.infCte.compl.ObsFisco:
                    return super().add(xTexto=xTexto, xCampo=xCampo)
        ...
        compl: compl = Element(compl, documentation=['Dados complementares da GTV-e para fins operacionais ou comerciais'])
        ...
        emit: emit = Element(emit, documentation=['Identificação do Emitente da GTV-e '])
...
Poderá não ser informado para os CT-e de redespacho intermediário e serviço vinculado a multimodal. Nos demais casos deverá sempre ser informado."""
        ...
        rem: rem = Element(rem, documentation=['Informações do Remetente ', 'Poderá não ser informado para os CT-e de redespacho intermediário e serviço vinculado a multimodal. Nos demais casos deverá sempre ser informado.'])
...
Poderá não ser informado para os CT-e de redespacho intermediário e serviço vinculado a multimodal. Nos demais casos deverá sempre ser informado."""
            ...
            email: TEmail = Element(TEmail, documentation=['Endereço de email'])
        dest: dest = Element(dest, documentation=['Informações do Destinatário ', 'Poderá não ser informado para os CT-e de redespacho intermediário e serviço vinculado a multimodal. Nos demais casos deverá sempre ser informado.'])
        origem: TEndeEmi = Element(TEndeEmi, documentation=['Informações do endereço da origem do serviço'])
        destino: TEndeEmi = Element(TEndeEmi, documentation=['Informações do endereço do destino do serviço'])

        class detGTV(ComplexType):
            ...
            class infEspecie(ComplexType):
                ...
                def add(self, tpEspecie=None, vEspecie=None, tpNumerario=None, xMoedaEstr=None) -> TGTVe.infCte.detGTV.infEspecie:
                    return super().add(tpEspecie=tpEspecie, vEspecie=vEspecie, tpNumerario=tpNumerario, xMoedaEstr=xMoedaEstr)

                tpEspecie: str = Element(str, documentation=['Tipo da Espécie', '1 - Cédula\n2 - Cheque\n3 - Moeda\n4 - Outros'])
                ...
                tpNumerario: str = Element(str, documentation=['Nacionalidade do Numerário', '1 - Nacional\n2 - Estrangeiro'])
                xMoedaEstr: str = Element(str, documentation=['Nome da Moeda', 'Informar somente se tipo de numerário for 2 - Estrangeiro'])
            ...
            class infVeiculo(ComplexType):
                """Grupo de informações dos veículos utilizados no transporte de valores"""
                _max_occurs = -1

                def add(self, placa=None, UF=None, RNTRC=None) -> TGTVe.infCte.detGTV.infVeiculo:
                    return super().add(placa=placa, UF=UF, RNTRC=RNTRC)
            ...
            infVeiculo: List[infVeiculo] = Element(infVeiculo, max_occurs=-1, documentation=['Grupo de informações dos veículos utilizados no transporte de valores'])
        detGTV: detGTV = Element(detGTV, documentation=['Grupo de informações detalhadas da GTV-e '])
...
Informar CNPJ ou CPF. Preencher os zeros não significativos."""
            ...
            def add(self, CNPJ=None, CPF=None) -> TGTVe.infCte.autXML:
    ...
    infCte: infCte = Element(infCte, documentation=['Informações do CT-e do tipo GTV-e'])
    ...
    infCTeSupl: infCTeSupl = Element(infCTeSupl, documentation=['Informações suplementares da GTV-e'])
...
class TProtGTVe(Element):
    """Tipo Protocolo de status resultado do processamento da GTV-e (Modelo 64)"""

    class infProt(ComplexType):
        """Dados do protocolo de status"""
        tpAmb: TAmb = Element(TAmb, documentation=['Identificação do Ambiente:\n1 - Produção\n2 - Homologação'])
        verAplic: TVerAplic = Element(TVerAplic, documentation=['Versão do Aplicativo que processou a GTV-e'])
        chCTe: TChDFe = Element(TChDFe, documentation=['Chaves de acesso da CT-e'])
        dhRecbto: TDateTimeUTC = Element(TDateTimeUTC, base_type=datetime, documentation=['Data e hora de processamento, no formato AAAA-MM-DDTHH:MM:SS TZD. '])
        nProt: TProt = Element(TProt, documentation=['Número do Protocolo de Status da GTV-e'])
        digVal: DigestValueType = Element(DigestValueType, documentation=['Digest Value da GTV-e processado. Utilizado para conferir a integridade da GTV-e original.'])
        cStat: str = Element(str, documentation=['Código do status da GTV-e.'])
        xMotivo: TMotivo = Element(TMotivo, documentation=['Descrição literal do status da GTV-e.'])
        Id: str = Attribute(ID)
    infProt: infProt = Element(infProt, documentation=['Dados do protocolo de status'])
...
class TRetGTVe(Element):
    ...
    cStat: TStat = Element(TStat, documentation=['código do status do retorno da consulta.'])
    xMotivo: TMotivo = Element(TMotivo, documentation=['Descrição literal do status do do retorno da consulta.'])
    protCTe: TProtGTVe = Element(TProtGTVe, documentation=['Reposta ao processamento do CT-e'])
    versao: str = Attribute(TVerCTe)
...
class TFinCTe(str):
    ...
    _restriction = Restriction(base=r"xs:string", white_space=r"preserve", enumeration=['0', '1', '3'])
    pass
...
class TCRT(str):
    ...
    _restriction = Restriction(base=r"xs:string", white_space=r"preserve", min_length=r"1", max_length=r"1", enumeration=['1', '2', '3', '4'])
    pass
...
Informar o nome do campo no atributo xCampo e o conteúdo do campo no XTexto"""
        ...
        class emit(ComplexType):
            ...
            IE: str = Element(str, documentation=['Inscrição Estadual do Emitente', 'A IE do emitente somente ficará sem informação para o caso do Regime Especial da NFF (tpEmis=3)'])
            IEST: str = Element(str, documentation=['Inscrição Estadual do Substituto Tributário'])
            xNome: str = Element(str, documentation=['Razão social ou Nome do emitente'])
            xFant: str = Element(str, documentation=['Nome fantasia'])
            enderEmit: TEndeEmi = Element(TEndeEmi, documentation=['Endereço do emitente'])
            CRT: TCRT = Element(TCRT, documentation=['Código do Regime Tributário', 'Informar: 1=Simples Nacional; \n2=Simples Nacional, excesso sublimite de receita bruta;\n3=Regime Normal. \n4=Simples Nacional - Microempreendedor Individual – MEI.\n'])
...
Este grupo deve ser informado quando o documento originário for NF"""
                ...
                class emiDocAnt(ComplexType):
                    ...
                    class idDocAnt(ComplexType):
                        ...
                        class idDocAntEle(ComplexType):
                            ...
                            chCTe: TChDFe = Element(TChDFe, documentation=['Chave de acesso do CT-e'])
            ...
            class infModal(ComplexType):
                ...
                versaoModal: str = Attribute(None, default='4.00')
                from .cteModalRodoviario_v400 import rodo
            ...
            class infServVinc(ComplexType):
                ...
                class infCTeMultimodal(ComplexType):
                    ...
                    chCTeMultimodal: TChDFe = Element(TChDFe, documentation=['Chave de acesso do CT-e Multimodal'])
        ...
        class infCteComp(ComplexType):
            ...
            def add(self, chCTe=None) -> TCTe.infCte.infCteComp:
                return super().add(chCTe=chCTe)

            chCTe: TChDFe = Element(TChDFe, documentation=['Chave do CT-e complementado'])
        infCteComp: List[infCteComp] = Element(infCteComp, max_occurs=10, documentation=['Detalhamento do CT-e complementado'])
...
Para Transporte de Pessoas indicar número de passageiros, para excesso de bagagem e transporte de valores indicar número de Volumes/Malotes"""
            ...
            class infDocRef(ComplexType):
                ...
                def add(self, nDoc=None, serie=None, subserie=None, dEmi=None, vDoc=None, chBPe=None) -> TCTeOS.infCte.infCTeNorm.infDocRef:
                    return super().add(nDoc=nDoc, serie=serie, subserie=subserie, dEmi=dEmi, vDoc=vDoc, chBPe=chBPe)
                ...
                chBPe: str = Element(str, documentation=['Chave de acesso do BP-e que possui eventos excesso de bagagem'])
...
Obrigatório para Pessoas e Bagagem"""
            ...
            class infGTVe(ComplexType):
                """Informações das GTV-e relacionadas ao CT-e OS"""
                _max_occurs = -1

                def add(self, chCTe=None, Comp=None) -> TCTeOS.infCte.infCTeNorm.infGTVe:
                    return super().add(chCTe=chCTe, Comp=Comp)

                chCTe: str = Element(str, documentation=['Chave de acesso da GTV-e'])

                class Comp(ComplexType):
                    """Componentes do Valor da GTVe"""
                    _max_occurs = -1

                    def add(self, tpComp=None, vComp=None, xComp=None) -> TCTeOS.infCte.infCTeNorm.infGTVe.Comp:
                        return super().add(tpComp=tpComp, vComp=vComp, xComp=xComp)

                    tpComp: str = Element(str, documentation=['Tipo do Componente', '1-Custodia\n2-Embarque\n3-Tempo de espera\n4-Malote\n5-Ad Valorem\n6-Outros'])
                    vComp: TDec_1302 = Element(TDec_1302, tipo="N", tam=(13, 2), base_type=Decimal, documentation=['Valor do componente'])
                    xComp: str = Element(str, documentation=['Nome do componente (informar apenas para outros)', 'Exemplos: FRETE PESO, FRETE VALOR, SEC/CAT, ADEME, AGENDAMENTO, etc'])
                Comp: List[Comp] = Element(Comp, max_occurs=-1, documentation=['Componentes do Valor da GTVe'])
            infGTVe: List[infGTVe] = Element(infGTVe, max_occurs=-1, documentation=['Informações das GTV-e relacionadas ao CT-e OS'])
        ...
        class infCteComp(ComplexType):
            ...
            def add(self, chCTe=None) -> TCTeOS.infCte.infCteComp:
                return super().add(chCTe=chCTe)
