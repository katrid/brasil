...
class TInutCTe(Element):
    ...
    class infInut(ComplexType):
        ...
        mod: TModCT = Element(TModCT, documentation=['Modelo da CT-e (57)'])
...
class TRetInutCTe(Element):
    ...
    class infInut(ComplexType):
        ...
        nCTIni: TNF = Element(TNF, documentation=['Número da CT-e inicial'])
        nCTFin: TNF = Element(TNF, documentation=['Número da CT-e final'])
        dhRecbto: dateTime = Element(dateTime, documentation=['Data e hora de recebimento, no formato AAAA-MM-DDTHH:MM:SS. Deve ser preenchida com data e hora da gravação no Banco em caso de Confirmação. Em caso de Rejeição, com data e hora do recebimento do Pedido de Inutilização.'])
