    ...
    class infEvento(ComplexType):
        ...
        def CNPJCPF(self, value):
            ...
            if len(value) == 11:
                ...
                self.CNPJ = value
        cOrgao: TCOrgaoIBGE = Element(TCOrgaoIBGE, documentation=['Código do órgão de recepção do Evento. Utilizar a Tabela do IBGE extendida, utilizar 90 para identificar SUFRAMA, 91 para RFB,  93 para ONE, 94 para SVBA'])
        tpAmb: TAmb = Element(TAmb, documentation=['Identificação do Ambiente:\n1 - Produção\n2 - Homologação'])
        CNPJ: TCnpj = Element(TCnpj, filter=str.isdigit, documentation=['CNPJ do autor', 'Informar zeros não significativos'])
        CPF: TCpf = Element(TCpf, filter=str.isdigit, documentation=['CPF do Autor', 'Informar zeros não significativos. \nUsar com serie específica 920-969 para emitente pessoa física com inscrição estadual, ou para emissor TAC do Regime Especial da Nota Fiscal Fácil'])
        chMDFe: TChMDFe = Element(TChMDFe, documentation=['Chave de Acesso do MDF-e vinculado ao evento'])
        dhEvento: TDateTimeUTC = Element(TDateTimeUTC, base_type=datetime, documentation=['Data e Hora do Evento, formato AAAA-MM-DDThh:mm:ss TZD'])
        tpEvento: string = Element(string, documentation=['Tipo do Evento:\n110111 - Cancelamento\n110112 - Encerramento\n110114 - Inclusão de Condutor\n310620 - Registro de Passagem\n510620 - Registro de Passagem BRId'])
        ...
        class detEvento(ComplexType):
            ...
            versaoEvento: str = Attribute(TVerEvento)
...
class TRetEvento(Element):
    ...
    class infEvento(ComplexType):
        ...
        cOrgao: TCOrgaoIBGE = Element(TCOrgaoIBGE, documentation=['Código do órgão de recepção do Evento. Utilizar a Tabela do IBGE extendida, utilizar 90 para identificar SUFRAMA, 91 RFB, 92 BackOffice BRId e 93 ONE'])
        ...
        chMDFe: TChMDFe = Element(TChMDFe, documentation=['Chave de Acesso MDF-e vinculado'])
        ...
        dhRegEvento: TDateTimeUTC = Element(TDateTimeUTC, base_type=datetime, documentation=['Data e Hora de do recebimento do evento ou do registro do evento formato AAAA-MM-DDThh:mm:ss TZD'])
...
class TProcEvento(Element):
    """Tipo procEvento"""
    eventoMDFe: TEvento = Element(TEvento)
    retEventoMDFe: TRetEvento = Element(TRetEvento)
    versao: str = Attribute(TVerEvento)
