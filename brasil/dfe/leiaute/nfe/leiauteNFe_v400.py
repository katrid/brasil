...
class TLocal(Element):
    ...
    xPais: str = Element(str, documentation=['Nome do país'])
    fone: str = Element(str, filter=str.isdigit, documentation=['Telefone, preencher com Código DDD + número do telefone , nas operações com exterior é permtido informar o código do país + código da localidade + número do telefone'])
    email: str = Element(str, documentation=['Informar o e-mail do expedidor/Recebedor. O campo pode ser utilizado para informar o e-mail de recepção da NF-e indicada pelo expedidor'])
    IE: TIe = Element(TIe, filter=str.isdigit, documentation=['Inscrição Estadual (v2.0)'])
...
class TInfRespTec(Element):
    ...
    CNPJ: TCnpjOpc = Element(TCnpjOpc, filter=str.isdigit, documentation=['CNPJ'])
    xContato: str = Element(str, documentation=['Informar o nome da pessoa a ser contatada na empresa desenvolvedora do sistema utilizado na emissão do documento fiscal eletrônico.'])
    email: str = Element(str, documentation=['Informar o e-mail da pessoa a ser contatada na empresa desenvolvedora do sistema.'])
    fone: str = Element(str, filter=str.isdigit, documentation=['Informar o telefone da pessoa a ser contatada na empresa desenvolvedora do sistema. Preencher com o Código DDD + número do telefone.'])
    idCSRT: str = Element(str, documentation=['Identificador do CSRT utilizado para montar o hash do CSRT'])
    hashCSRT: str = Element(str, documentation=['O hashCSRT é o resultado da função hash (SHA-1 – Base64) do CSRT fornecido pelo fisco mais a Chave de Acesso da NFe.'])
    ...
    class infNFe(ComplexType):
        ...
        class ide(ComplexType):
            ...
            class NFref(ComplexType):
                ...
                class refNF(ComplexType):
                    ...
                    mod: str = Element(str, documentation=['Código do modelo do Documento Fiscal. Utilizar 01 para NF modelo 1/1A e 02 para NF modelo 02'])
                    serie: TSerie = Element(TSerie, documentation=['Série do Documento Fiscal, informar zero se inexistente'])
                    nNF: TNF = Element(TNF, documentation=['Número do Documento Fiscal'])
                refNF: refNF = Element(refNF, documentation=['Dados da NF modelo 1/1A referenciada ou NF modelo 2 referenciada'])
        ...
        class det(ComplexType):
            ...
            class prod(ComplexType):
                ...
                cBarra: str = Element(str, documentation=['Codigo de barras diferente do padrão GTIN'])
                ...
                CNPJFab: TCnpj = Element(TCnpj, filter=str.isdigit, documentation=['CNPJ do Fabricante da Mercadoria, obrigatório para produto em escala NÃO relevante.'])
...
Substituição Tributaria;"""
        ...
        class pag(ComplexType):
            ...
            class detPag(ComplexType):
                ...
                class card(ComplexType):
                    ...
                    CNPJ: TCnpj = Element(TCnpj, filter=str.isdigit, documentation=['CNPJ da instituição de pagamento'])
                    tBand: str = Element(str, documentation=['Bandeira da operadora de cartão de crédito. Utilizar a Tabela de Códigos das Operadoras de cartão de crédito e/ou débito publicada no Portal Nacional da Nota Fiscal Eletrônica.  '])
        ...
        class infIntermed(ComplexType):
            ...
            CNPJ: TCnpj = Element(TCnpj, filter=str.isdigit, documentation=['CNPJ do Intermediador da Transação (agenciador, plataforma de delivery, marketplace e similar) de serviços e de negócios.'])
            idCadIntTran: str = Element(str, documentation=['Identificador cadastrado no intermediador'])
        infIntermed: infIntermed = Element(infIntermed, documentation=['Grupo de Informações do Intermediador da Transação'])
...
e o conteúdo do campo no xTexto"""
    ...
    class infNFeSupl(ComplexType):
        ...
        urlChave: str = Element(str, documentation=['Informar a URL da "Consulta por chave de acesso da NFC-e". A mesma URL que deve estar informada no DANFE NFC-e para consulta por chave de acesso.'])
...
class TProtNFe(Element):
    ...
    class infProt(ComplexType):
        ...
        cMsg: str = Element(str, documentation=['Código da Mensagem.'])
        xMsg: str = Element(str, documentation=['Mensagem da SEFAZ para o emissor.'])
        Id: str = Attribute(ID)
